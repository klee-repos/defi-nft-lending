...
const PRICE_FEED_INITIAL_PRICE = ethers.utils.parseUnits(
  "1000",
  PRICE_FEED_DECIMALS
);
...
const BORROW_AMOUNT_ETH = ethers.utils.parseEther(".11");
const DEPOSIT_AMOUNT_ETH = ethers.utils.parseEther("0.3");
const PAY_BACK_ETH = ethers.utils.parseEther("0.1");
const LOAN_DURATION = 30;

async function calculateExpectedMaxBorrow(NFTLending, SimpleNFT) {
  let totalTokenIds = await NFTLending.s_accountsToTotalNFTDeposits(
    WALLET_ADDRESS,
    SimpleNFT.address
  );
  let expectedCollateralUSD = await NFTLending.ethToUSD(
    `${NFT_PROJECT_ETH_FLOOR * totalTokenIds}`
  );
  let expectedMaxBorrow = (expectedCollateralUSD * BORROW_POWER) / 100;
  return expectedMaxBorrow;
}
...
describe("DeFi NFTLending unit tests", function () {
  ...
  describe("account info", function () {
    ...
    it("verify max borrow", async () => {
      let maxBorrowUSD = await NFTLending.borrowMaxUSD(WALLET_ADDRESS);
      let expectedMaxBorrow = await calculateExpectedMaxBorrow(
        NFTLending,
        SimpleNFT
      );
      assert(
        expectedMaxBorrow.toString() === maxBorrowUSD.toString(),
        "max borrow incorrect"
      );
    });
  });
  ...
  describe("borrow some eth", function () {
    it("borrow", async () => {
      ...
      let interest = (Number(BORROW_AMOUNT_ETH) * BORROW_INTEREST_RATE) / 100;
      TreasuryBalance = await NFTLending.s_treasuryEth();
      let expectedTreasury =
        Number(DEPOSIT_AMOUNT_ETH) -
        Number(BORROW_AMOUNT_ETH) -
        Number(interest);
      assert(
  ...
  describe("try to withdraw nft", function () {
    it("withdraw nft from NFTLending", async () => {
      ...
      let txReceipt = await tx.wait();
      assert(txReceipt.events[0].event === "DeleteDeposit", "withdraw failed");
    });
